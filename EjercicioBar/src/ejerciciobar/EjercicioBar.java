...
public class EjercicioBar {
    ...
    public static void compruebaDomingo(float[] arrayPasta) {
        ...
        for (int i = 0; i < arrayPasta.length; i++) {
            mediaSemanaMenosElDomingo += arrayPasta[i];
        }
        mediaSemanaMenosElDomingo /= arrayPasta.length;
        if (mediaSemanaMenosElDomingo < arrayPasta[arrayPasta.length - 1]) {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        File archivo = new File("src/ejerciciobar/datos");
