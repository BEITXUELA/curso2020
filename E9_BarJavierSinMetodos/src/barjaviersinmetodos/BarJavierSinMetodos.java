package barjaviersinmetodos;

import java.io.File;
import java.io.FileNotFoundException;
import java.util.Scanner;

public class BarJavierSinMetodos {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        File archivo = new File("src/barjaviersinmetodos/datos");
        ...
        int posicionMenor = 0;
        int posicionMayor = 0;
        float mediaSemanaMenosElDomingo = 0;
        do {
            ...
            if (!entradaIncorrecta) {
                //CALCULAMOS EL MENOR Y EL MAYOR
                ...
                //CALCULAMOS EL MENOR Y EL MAYOR
                //CALCULAMOS LA MEDIA Y COMPARAMOS CON EL DOMINGO

                for (int i = 0; i < arrayGanancias.length; i++) {
                    mediaSemanaMenosElDomingo += arrayGanancias[i];
                }
                mediaSemanaMenosElDomingo /= arrayGanancias.length;
                if (mediaSemanaMenosElDomingo < arrayGanancias[arrayGanancias.length - 1]) {
            ...
            arrayGanancias = new float[6];
            menor = arrayGanancias[0];
            mayor = arrayGanancias[0];
            posicionMenor = 0;
            posicionMayor = 0;
            mediaSemanaMenosElDomingo = 0;
